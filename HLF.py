...
COLUMNAS = 10
MAR = " "
barco_1 = "Ber" #Bergantin
barco_2 = "Cañ" #Cañonero
barco_3 = "Aco" #Acorazado
barco_4 = "Uss" #USS Abraham Lincoln
DISPARO_FALLADO = "0"
DISPARO_ACERTADO = "X"
...
def colocar_e_imprimir_barcos(matriz, cantidad_barcos, jugador):
            # Asignamos la cantidad de cada barco
            barcos_una_celda = cantidad_barcos == 4
            barcos_dos_celdas_verticales = cantidad_barcos == 2
            barcos_dos_celdas_horizontales = cantidad_barcos == 1
            barco_tres_celdas_horizontal= cantidad_barcos == 1
            barco_tres_celdas_vertical = cantidad_barcos == 1
            barco_cuatro_celdas = cantidad_barcos == 1
            if jugador == JUGADOR_1:
                print("Imprimiendo barcos del jugador 1 ")
            else:
                print("Imprimiendo barcos del jugador 2 ")
            print(f"Barcos de una celda: {barcos_una_celda}\nBarcos verticales de dos celdas: {barcos_dos_celdas_verticales}\nBarcos horizontales de dos celdas: {barcos_dos_celdas_horizontales}\nBarcos horizontales de tres celdas:{barco_tres_celdas_horizontal}\nBarcos verticales de tres celdas:{barco_tres_celdas_vertical}\nBarco de cuatro celdas:{barco_cuatro_celdas}\nTotal: {barcos_una_celda+barcos_dos_celdas_verticales+barcos_dos_celdas_horizontales+barco_tres_celdas_horizontal+barco_tres_celdas_vertical+barco_cuatro_celdas}")
            # Primero colocamos los de cuatro celdas 
            matriz = colocar_barco_de_cuatro_celdas(barco_cuatro_celdas, barco_4, matriz
                                                    )
            matriz = colocar_barcos_de_tres_celdas_horizontal(barco_tres_celdas_horizontal, barco_3,matriz)
            matriz = colocar_barcos_de_tres_celdas_vertical(barco_tres_celdas_vertical, barco_3,matriz)
            matriz = colocar_barcos_de_dos_celdas_horizontal(
                barcos_dos_celdas_horizontales, barco_2, matriz)
            matriz = colocar_barcos_de_dos_celdas_vertical(
                barcos_dos_celdas_verticales, barco_2, matriz)
            matriz = colocar_barcos_de_una_celda(barcos_una_celda, barco_1, matriz)
            return matriz
...
def colocar_barcos_de_tres_celdas_horizontal(cantidad, tipo_barco, matriz):
            ...
            while True:
                ...
                if barcos_colocados >= cantidad:
                    break
            return matriz
def colocar_barco_de_cuatro_celdas(cantidad, tipo_barco, matriz):
            barcos_colocados = 0
            while True:
                x = obtener_x_aleatoria()
                y = obtener_y_aleatoria()
                ...
                y3 = y2+1
                y4 = y3+1
                if coordenada_en_rango(x, y) and coordenada_en_rango(x, y2) and coordenada_en_rango (x,y3) and coordenada_en_rango(x,y4) and es_mar(x, y, matriz) and es_mar(x, y2, matriz) and es_mar(x,y3, matriz):
                    matriz[y][x] = tipo_barco
                    matriz[y2][x] = tipo_barco
                    matriz[y3][x] = tipo_barco
...
def solicitar_coordenadas(jugador):
            ...
            # Este ciclo se rompe cuando ingresan una fila correcta
            y = None
            x = None
            while True:
                letra_fila = input(
                    "Ingresa la letra de la fila tal y como aparece en el tablero: ")
                # Necesitamos una letra de 1 carácter. Si no es de 1 carácter usamos continue
                if len(letra_fila) != 1:
            ...
            for y in range(FILAS):
                for x in range(COLUMNAS):
                    ...
                    if celda != MAR and celda != DISPARO_ACERTADO and celda != DISPARO_FALLADO:
                        return False
            # En este punto todos los barcos han sido hundidos
            return True
...
def acerca_de():
            print("Programado por Daniel Manso")
